# https://github.com/g0vhk-io/HKAddressParser
import bisect
import json
import requests
import re
...
class Address:
    ...
    def queryOGCIO(self, RequestAddress, n):
        ...
        r = session.get(base_url, headers=headers, params={"q": RequestAddress, "n": n})
    ...
    def removeFloor(self, inputAddress):
        return re.sub(
            "([0-9A-z\-\s]+[樓層]|[0-9A-z號\-\s]+[舖鋪]|地[下庫]|平台).*", "", inputAddress
        )
...
def matchStr(inAddr, fieldName, inStr):
    ...
    for i in range(0, len(inStr)):
        ...
        if (len(inStr) - i) <= 3:
            break  # give up if remaining inStr too short
        if i >= len(inStr) // 2:
            break  # give up if already stripped half
...
def matchChiStreetOrVillage(inAddr, inDict):
    ...
    key = None
    if "StreetName" in inDict:
        key = "StreetName"
    if "VillageName" in inDict:
        key = "VillageName"
    ...
    if matchedPos != None:
        ...
        reResult = re.match(
            "([0-9A-z]+)[至及\-]*([0-9A-z]*)號", inStr
        )  # this matches '591-593號QWER'
    ...
    if ogcioBNoTo == "":
        ogcioBNoTo = ogcioBNoFrom
    if inAddrBNoTo == "":
        inAddrBNoTo = inAddrBNoFrom
    ...
    if "BuildingNoFrom" in inDict:
        goodness = 1.0 if inAddrBNoFrom == ogcioBNoFrom else 0.5
        matches.append(("BuildingNoFrom", ogcioBNoFrom, inAddrBNoSpan, goodness))
    if "BuildingNoTo" in inDict:
        goodness = 1.0 if inAddrBNoTo == ogcioBNoTo else 0.5
...
class Similarity:
    ...
    def __repr__(self):
        ...
        tmp = "".join(
            [s if self.inAddrHasMatch[i] else "?" for (i, s) in enumerate(self.inAddr)]
        )
        outStr += "match: %s\n" % tmp
...
def getSimilarityWithOGCIO(inAddr, ogcioResult):
    ...
    for (fieldName, fieldVal, matchSpan, goodness) in matches:
        ...
        for i in range(matchSpan[0], matchSpan[1]):
            inAddrHasMatch[i] = True
